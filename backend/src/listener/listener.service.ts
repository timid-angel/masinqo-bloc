...
export class ListenerService {
  ...
  async getOneListener(req: Request) {
    const listener = await this.parseToken(req, this.listenerModel, 2);
    if (!listener) {
      throw new UnauthorizedException('Invalid token - Listener not found');
    }
    const res = {};
    res['name'] = listener.name;
    // res["email"] = listener.email;
    // res["profilePicture"] = listener.profilePicture;
    // res["albums"] = await this.albumModel.find({artist: listener._id.valueOf()});
  ...
  async update(req: Request) {
    ...
    if ('email' in reqBody && reqBody['email'].length > 0) {
      const cand = await this.listenerModel.find({ email: reqBody['email'] });
      if (cand.length > 0) {
  ...
  async remove(req: Request, id: string) {
    ...
    if (!listener) {
      throw new BadRequestException(
        'Listener with the provided ID does not exist.',
      );
    }
  ...
  async addFavorite(req: Request, id: string) {
    ...
    if (!album) {
      throw new BadRequestException(
        'Album with the provided ID does not exist.',
      );
    }
