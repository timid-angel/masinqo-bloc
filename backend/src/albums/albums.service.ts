...
export class AlbumsService {
  ...
  async uploadAlbumImage(id: string, file: Express.Multer.File) {
    if (!file) {
      throw new BadRequestException(
        'File not selected. Select a file to upload',
      );
    }
    ...
    if (!imageExtensions.includes(fileExtension)) {
      throw new BadRequestException(
        'Invalid file format. Only png, jpg, and jpeg formats are valid',
      );
    }
  ...
  async addSong(id: string, req: Request, file: Express.Multer.File) {
    ...
    if (artistFromToken._id.valueOf() !== album.artist) {
      throw new UnauthorizedException(
        'Only creators of an album can add songs',
      );
    }
    ...
    if (duplicateName) {
      try {
        ...
        throw new BadRequestException(
          'A song with a similar name already exists on this album',
        );
      } catch (err) {
  ...
  async uploadSong(id: string, file: Express.Multer.File, songIndex) {
    ...
    if (!audioExtensions.includes(fileExtension)) {
      throw new BadRequestException(
        'invalid file format only mp3 and wav formats are valid',
      );
    }
  ...
  async removeSong(id: string, req: Request) {
    ...
    if (artistFromToken._id.valueOf() !== album.artist) {
      throw new UnauthorizedException(
        'Only creators of an album can remove songs',
      );
    }
    let found = false;
  ...
  async getSong(albumId: string, idx: string) {
    if (isNaN(parseInt(idx)))
      throw new BadRequestException('Invalid song index - non numeric index');
